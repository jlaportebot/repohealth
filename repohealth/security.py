...
SECRET_PATTERNS = [
    (re.compile(r'(?:password|passwd|pwd)\s*[:=]\s*["\'][^"\']{4,}["\']', re.I), "Hardcoded password"),
    (re.compile(r'(?:api_?key|apikey|api_secret)\s*[:=]\s*["\'][^"\']{8,}["\']', re.I), "Hardcoded API key"),
    (re.compile(r'(?:secret|token|auth_token)\s*[:=]\s*["\'][^"\']{8,}["\']', re.I), "Hardcoded secret/token"),
    (re.compile(r'(?:aws_access_key_id|aws_secret_access_key)\s*[:=]\s*["\'][^"\']+["\']', re.I), "AWS credential"),
    (re.compile(r'(?:private_key)\s*[:=]\s*["\']-----BEGIN', re.I), "Embedded private key"),
    (re.compile(r'mongodb(?:\+srv)?://[^:\s]+:[^@\s]+@', re.I), "MongoDB URI with credentials"),
    (re.compile(r'postgres(?:ql)?://[^:\s]+:[^@\s]+@', re.I), "PostgreSQL URI with credentials"),
...
def check(repo_path: str | None = None, max_findings: int = 50) -> SecurityResult:
    ...
    has_renovate = (base / "renovate.json").exists() or (base / ".github" / "renovate.json").exists()
        ...
        if audit_check.returncode == 0:
            ...
            if req_file:
                ...
                if audit_run.returncode == 0:
                    ...
                    pip_audit_result = f"{vuln_count} known vulnerabilities" if vuln_count else "Audit completed with warnings"
